            ...
            <div class="nk-content-body">
                <div class="components-preview mx-auto">
                    <div class="nk-block nk-block-lg">
                                    ...
                                    <div class="row">
                                        ...
                                        <div class="col-lg-3 col-sm-4">
                                            <select class="form-select js-select2" title="Тип затрата" placeholder="Select Multiple options" name="type" required data-search="on">
                                                <option {{ $type == 0 ? 'selected' : NULL }} value="0">Все</option>
                                                @foreach($contracts as $contract)
                                                    <option {{ $type == $contract->id ? 'selected' : NULL }} value="{{ $contract->id }}">{{ $contract->name }}</option>
                                                @endforeach
                                            </select>
                                        </div>
                                        @hasanyrole('admin')
                        ...
                        <div class="card">
                            <h4 class="text-center mb-2 mt-2">{{ $tname->name }}: {{ number_format($data->sum('price'), 0, '.', ' ') }} сум</h4>
