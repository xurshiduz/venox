    ...
    <div class="container-fluid">
        <div class="nk-content-inner">
            <div class="nk-content-body">
                        ...
                        <div class="col-sm-12 col-lg-12 col-xxl-12">
                            <div class="card card-bordered">
                                <div class="card-inner">
                                    <div class="team">
                                        <ul class="team-info" style="padding: 0">
                                            @php($qarzlar = App\Models\Checkout::where('checkout_tip_id', 1)->sum('total_price_debt'))
                                            <li><span>Долг</span><span><b>{{ number_format($qarzlar, 0, '.', ' ') }}</b> сум</span></li>
                                        </ul>
                                    </div><!-- .team -->
                                </div><!-- .card-inner -->
                            </div><!-- .card -->
                            ...
                            <div class="card card-bordered">
                                <div class="card-inner">
                                    <div class="team">
                                        ...
                                        <ul class="team-info">
                                            @php($aktbugun = App\Models\CashReceipt::where('status', 1)->whereDate('date', Carbon\Carbon::today())->sum('price'))
                                            @php($aktmonth = App\Models\CashReceipt::where('status', 1)->whereBetween('date', [Carbon\Carbon::now()->startOfMonth(), Carbon\Carbon::now()->endOfMonth()])->sum('price'))
                                            @php($aktsubmonth = App\Models\CashReceipt::where('status', 1)->whereBetween('date', [Carbon\Carbon::now()->subMonth()->startOfMonth(), Carbon\Carbon::now()->subMonth()->endOfMonth()])->sum('price'))
                                            @php($aktsubyear = App\Models\CashReceipt::where('status', 1)->whereBetween('date', [Carbon\Carbon::now()->subYear()->startOfYear(), Carbon\Carbon::now()->subYear()->endOfYear()])->sum('price'))
                                            @php($aktyear = App\Models\CashReceipt::where('status', 1)->whereBetween('date', [Carbon\Carbon::now()->startOfYear(), Carbon\Carbon::now()->endOfYear()])->sum('price'))
                                            ...
                                            <li><span>Прошлого мес. ({{ Str::limit(Carbon\Carbon::now()->subMonth()->locale('ru_RU')->monthName, 3, '') }})</span><span><b>{{ number_format($aktsubmonth, 0, '.', ' ') }}</b> сум</span></li>
                                            <li><span>Прошлого года ({{ Carbon\Carbon::now()->subYear()->format('Y') }})</span><span><b>{{ number_format($aktsubyear, 0, '.', ' ') }}</b> сум</span></li>
                                            <li><span>С начало года ({{ Carbon\Carbon::now()->format('Y') }})</span><span><b>{{ number_format($aktyear, 0, '.', ' ') }}</b> сум</span></li>
                                        </ul>
                                        <div class="team-view">
                                            <a href="{{ route('dashboard_month', ['type' => 'kassa']) }}" class="btn btn-block btn-dim btn-primary"><span>Просмотр по месяцам</span></a>
                                        </div>
                                    </div><!-- .team -->
                                </div><!-- .card-inner -->
                            </div><!-- .card -->
                                ...
                                <div class="card-inner">
                                    <div class="team">
                                        ...
                                        <ul class="team-info">
                                            ...
                                            @php($dolgsubmonth = App\Models\Checkout::where('checkout_tip_id', 1)->whereBetween('date', [Carbon\Carbon::now()->subMonth()->startOfMonth(), Carbon\Carbon::now()->subMonth()->endOfMonth()])->sum('total_price_debt'))
                                            @php($dolgsubyear = App\Models\Checkout::where('checkout_tip_id', 1)->whereBetween('date', [Carbon\Carbon::now()->subYear()->startOfYear(), Carbon\Carbon::now()->subYear()->endOfYear()])->sum('total_price_debt'))
                                            @php($dolgyear = App\Models\Checkout::where('checkout_tip_id', 1)->whereBetween('date', [Carbon\Carbon::now()->startOfYear(), Carbon\Carbon::now()->endOfYear()])->sum('total_price_debt'))
                                            ...
                                            <li><span>Прошлого мес. ({{ Str::limit(Carbon\Carbon::now()->subMonth()->locale('ru_RU')->monthName, 3, '') }})</span><span><b>{{ number_format($dolgsubmonth, 0, '.', ' ') }}</b> сум</span></li>
                                            <li><span>Прошлого года ({{ Carbon\Carbon::now()->subYear()->format('Y') }})</span><span><b>{{ number_format($dolgsubyear, 0, '.', ' ') }}</b> сум</span></li>
                                            <li><span>С начало года ({{ Carbon\Carbon::now()->format('Y') }})</span><span><b>{{ number_format($dolgyear, 0, '.', ' ') }}</b> сум</span></li>
                                        </ul>
                                        <div class="team-view">
                                            <a href="{{ route('dashboard_month', ['type' => 'dolg']) }}" class="btn btn-block btn-dim btn-primary"><span>Просмотр по месяцам</span></a>
                                        </div>
                                    </div><!-- .team -->
                                </div><!-- .card-inner -->
                            </div><!-- .card -->
                                ...
                                <div class="card-inner">
                                    <div class="team">
                                        ...
                                        <ul class="team-info">
                                            ...
                                            <li><span>Прошлого мес. ({{ Str::limit(Carbon\Carbon::now()->subMonth()->locale('ru_RU')->monthName, 3, '') }})</span><span><b>{{ number_format($aktsubmonth + $dolgsubmonth, 0, '.', ' ') }}</b> сум</span></li>
                                            <li><span>Прошлого года ({{ Carbon\Carbon::now()->subYear()->format('Y') }})</span><span><b>{{ number_format($aktsubyear + $dolgsubyear, 0, '.', ' ') }}</b> сум</span></li>
                                            <li><span>С начало года ({{ Carbon\Carbon::now()->format('Y') }})</span><span><b>{{ number_format($aktyear + $dolgyear, 0, '.', ' ') }}</b> сум</span></li>
                                        </ul>
                                        
                                        <div class="team-view">
                                            <a href="{{ route('dashboard_month', ['type' => 'all']) }}" class="btn btn-block btn-dim btn-primary"><span>Просмотр по месяцам</span></a>
                                        </div>
                                    </div><!-- .team -->
                                </div><!-- .card-inner -->
                            </div><!-- .card -->
                                ...
                                <div class="card-inner">
                                    <div class="team">
                                        ...
                                        <ul class="team-info">
                                            ...
                                            <li><span style="color: #6c6c6c;">Умумий</span><span><b>{{ number_format(App\Models\WarehouseStock::where('stock', '>', 0)->sum('stock'), 0, '.', ' ') }}</b> | {{ number_format($d->count(), 0, '.', ' ') }} SKU</span></li>
                                            @foreach(App\Models\Warehouse::where('status', 1)->whereIn('id', [1,2,3,4])->get() as $ware)
                                            <li><span style="color: #6c6c6c;">{{ $ware->name }}</span><span><b>{{ number_format($ware->stockall()->where('stock', '>', 0)->sum('stock'), 0, '.', ' ') }}</b> | {{ number_format($ware->stockall()->count(), 0, '.', ' ') }} SKU</span> </li>
                                            @endforeach
                                        </ul>
                                ...
                                <div class="card-inner">
                                    <div class="team">
                                        ...
                                        <ul class="team-info text">
                                            ...
                                            <li><span>{{ $manager->name }}</span><span>{{ number_format(App\Models\Checkout::where('checkout_tip_id', 1)->where('manager_id', $manager->id)->whereBetween('created_at', [Carbon\Carbon::now()->subMonths(3)->startOfMonth(), Carbon\Carbon::now()->endOfMonth()])->count(), 0, '.', ' ') }}</span></li>
                                            @endforeach
                ...
                </div>* Цены продаж и закупок по артикулам не включены.
            </div>
        </div>
    </div>
</div>
