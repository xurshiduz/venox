                    ...
                    <div class="table-responsive">
                        <table class="table table-striped">
                            ...
                            <tbody>
                                ...
                                @if($item->first()->prodid->checkindetails()->where('warehouse_id', $ware->id)->where('status', 1)->sum('qty') - $item->first()->prodid->checkoutdetails()->where('warehouse_id', $ware->id)->where('status', 1)->sum('qty') != 0)
                                <tr>
                                    <td>{{ $item->first()->prodid->barcode }}</td>
                                    <td>{{ $item->first()->prodid->name }}</td>
                                    <td>{{ $item->first()->prodid->checkindetails()->where('warehouse_id', $ware->id)->where('status', 1)->sum('qty') - $item->first()->prodid->checkoutdetails()->where('warehouse_id', $ware->id)->where('status', 1)->sum('qty') }}</td>
