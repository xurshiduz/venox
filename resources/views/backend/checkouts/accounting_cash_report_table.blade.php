...
<thead class="table-light"><tr><th>№</th><th>Sana</th><th>Agent</th><th>Tovar</th><th>Klient</th><th>Bonus / bez bonus</th><th>Prihod summa (USD)</th><th>Summa USD</th><th>KPI</th><th>Fiksa agent</th><th>Venox bonus kassa</th><th>Zavod kassa</th><th>Amal</th></tr></thead>
...
@if($rows->isNotEmpty())<tfoot class="table-light"><tr class="fw-bold"><td colspan="6" class="text-end">Umumiy jami:</td><td>{{ number_format($reportTotals->sum('purchase_cost_usd'),2,'.',' ') }}</td><td>{{ number_format($reportTotals->sum('payment_usd'),2,'.',' ') }}</td><td>{{ number_format($reportTotals->sum('kpi'),2,'.',' ') }}</td><td>{{ number_format($reportTotals->sum('agent_amount'),2,'.',' ') }}</td><td>{{ number_format($reportTotals->sum('venox'),2,'.',' ') }}</td><td>{{ number_format($reportTotals->sum('factory'),2,'.',' ') }}</td><td></td></tr></tfoot>@endif
</table></div>
