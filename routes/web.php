...
Route::get('/_maintenance/ravshan-debt-audit-f71ac8936e2b', function () {
    $clients = \App\Models\Client::query()
        ->where('name', 'like', '%Ravshan%')
        ->where('name', 'like', '%2%')
        ->get();
    ...
    return response()->json($clients->map(function ($client) use ($accounting) {
        ...
        $checkouts = \App\Models\Checkout::with('alldetails')
            ...
            ->orderBy('id')
            ->get();
        $payments = \App\Models\CashReceipt::with('checkout:id,client_id,currency_type,currency_type_price')
            ->where('status', 1)
            ...
            ->orderBy('id')
            ->get();
        $returns = \App\Models\Checkin::with('details')
            ->where('client_id', $client->id)
            ->where('type_id', 4)
            ->orderBy('date')
            ->orderBy('id')
        ...
        $checkoutRows = $checkouts->map(function ($checkout) {
            ...
            return [
                ...
                'number' => $checkout->number_work,
                'currency_type' => $checkout->currency_type,
        ...
        $paymentRows = $payments->map(function ($payment) use ($accounting) {
            $usd = $payment->checkout
                ? $accounting->paymentAmountToUsd(
                    ...
                    $payment->checkout->currency_type,
                    $payment->checkout->currency_type_price
                )
                : $accounting->legacyUnlinkedPaymentToUsd(
                    (float) $payment->price,
                    (int) $payment->currency_type,
                    (float) ($payment->currency_type_price ?: \App\Models\Currency::usdRateForDate($payment->date ?: $payment->created_at)),
            ...
            return [
                ...
                'rate' => $payment->currency_type_price,
                'comment' => $payment->comment,
                'usd' => $usd,
            ];
        });
        $returnRows = $returns->map(function ($return) {
            $raw = (float) $return->details->sum('total_price');
            return [
                'id' => $return->id,
                'date' => $return->date,
                'status' => $return->status,
                'raw' => $raw,
                'usd' => \App\Models\Currency::documentAmountToUsd($raw, (int) $return->currency_type, (float) $return->currency_type_price, $return->date),
            ];
        });
        $allSales = (float) $checkoutRows->sum('detail_total_usd');
        $activeSales = (float) $checkoutRows->where('status', 1)->sum('detail_total_usd');
        $activePayments = (float) $paymentRows->sum('usd');
        $allReturns = (float) $returnRows->sum('usd');
        $activeReturns = (float) $returnRows->where('status', 1)->sum('usd');
        ...
        return [
            'client' => [
                ...
                'rate' => $client->currency_type_price,
                'initial_usd' => $initialUsd,
            ],
            'totals' => [
                'all_sales_usd' => $allSales,
                'active_sales_usd' => $activeSales,
                'payments_usd' => $activePayments,
                'all_returns_usd' => $allReturns,
                'active_returns_usd' => $activeReturns,
                'current_export_closing_usd' => $initialUsd + $allSales - $activePayments - $allReturns,
                'active_only_closing_usd' => $initialUsd + $activeSales - $activePayments - $activeReturns,
            ],
